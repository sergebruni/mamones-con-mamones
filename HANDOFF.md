...
## Hecho recientemente
- **Animación al jugar carta (SP + MP):** la carta seleccionada vuela desde la mano al centro de la mesa. SP: capa `animLayer` + tween en `GameScene.animateCardToCenter`. MP: clon `position:fixed` con transición CSS (`.og__fly`) en `OnlineGame.jugarConAnim`. Solo se anima la carta propia (las del rival son anónimas). Regla viva: todo cambio de juego va a ambos motores → ver memoria [[paridad-sp-mp]].
